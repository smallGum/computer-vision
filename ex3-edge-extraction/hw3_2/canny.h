...
public:
  ...
  canny(CImg<unsigned char>& greyImg, float lowThreshold = 2.5f, float highthreshold = 7.5f, float gaussiankernelradius = 4.2f, int gaussiankernelwidth = 16, int contrastnormalised = 0);
  // get the final edge image
  CImg<unsigned char> getEdgeImg();

  // get the final edge image
  CImg<double> getXGradImg();
  // get the final edge image
  CImg<double> getYGradImg();
  // destructor
  ~canny();
};
...
canny::canny(CImg<unsigned char>& greyImg, float lowThreshold, float highthreshold, float gaussiankernelradius, int gaussiankernelwidth, int contrastnormalised) {
  ...
  xEdge = CImg<double>(width, height, 1, 1, 0);
  yEdge = CImg<double>(width, height, 1, 1, 0);
  // 定义3*3领域矩阵I
  CImg_3x3(I, double);
  // 遍历计算梯度值
  cimg_for3x3(greyImg, x, y, 0, 0, I, double) {
      const double ix = ((double)Inc - (double)Ipc + (double)Inp - (double)Ipp + (double)Inn - (double)Ipn) / 9.0;
      const double iy = ((double)Icp - (double)Icn + (double)Ipp - (double)Ipn + (double)Inp - (double)Inn) / 9.0;
      xEdge(x, y) = ix;
      yEdge(x, y) = iy;
  }
}
...
CImg<double> canny::getXGradImg() {
  return xEdge;
}

CImg<double> canny::getYGradImg() {
  return yEdge;
}
